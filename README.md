...
Le schéma, au format image SVG, de la base de données est généré avec Graphviz (`dot`).
La transformation du schéma de la base de données au format SQL au format graphique DOT est réalisé par le script `sql2graphviz`.
